...
import ir.mahdiparastesh.hellocharts.formatter.BubbleChartValueFormatter;
...
public class BubbleChartRenderer extends AbstractChartRenderer {
    ...
    /**
     * True if bubbleScale = bubbleScaleX so the renderer should used {@link ChartCalculator#computeRawDistanceX(float)}
     * , if false bubbleScale = bubbleScaleY and renderer should use
     * {@link ChartCalculator#computeRawDistanceY(float)}.
     */
    ...
    public void removeMargins() {
        ...
        final float pxX = calculator.computeRawDistanceX(maxRadius * bubbleScaleX);
        final float pxY = calculator.computeRawDistanceY(maxRadius * bubbleScaleY);
        final float scaleX = calculator.getMaximumViewport().width() / contentRect.width();
        final float scaleY = calculator.getMaximumViewport().height() / contentRect.height();
        float dx = 0;
        float dy = 0;
    ...
    private float processBubble(BubbleValue bubbleValue, PointF point) {
        ...
        if (isBubbleScaledByX) {
            radius *= bubbleScaleX;
            rawRadius = calculator.computeRawDistanceX(radius);
        } else {
            radius *= bubbleScaleY;
            rawRadius = calculator.computeRawDistanceY(radius);
        }
