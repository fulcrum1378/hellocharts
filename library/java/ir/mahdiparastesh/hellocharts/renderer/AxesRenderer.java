...
import ir.mahdiparastesh.hellocharts.util.AxisAutoValues;
...
public class AxesRenderer {
    ...
    private void insetContentRectWithAxesMargins(int axisMargin, int position) {
        if (LEFT == position) {
            chart.getChartCalculator().insetContentRect(axisMargin, 0, 0, 0);
        } else if (RIGHT == position) {
            chart.getChartCalculator().insetContentRect(0, 0, axisMargin, 0);
        } else if (TOP == position) {
            chart.getChartCalculator().insetContentRect(0, axisMargin, 0, 0);
        } else if (BOTTOM == position) {
            chart.getChartCalculator().insetContentRect(0, 0, 0, axisMargin);
        }
    }
    ...
    private void initAxisMeasurements(Axis axis, int position) {
        if (LEFT == position) {
            if (axis.isInside()) {
                labelBaselineTab[position] = calculator.getContentRectMinusAllMargins().left + axisMargin;
                nameBaselineTab[position] = calculator.getContentRectMinusAxesMargins().left - axisMargin
                        - labelTextDescentTab[position];
            } else {
                labelBaselineTab[position] = calculator.getContentRectMinusAxesMargins().left - axisMargin;
            ...
            separationLineTab[position] = calculator.getContentRectMinusAllMargins().left;
        } else if (RIGHT == position) {
            if (axis.isInside()) {
                labelBaselineTab[position] = calculator.getContentRectMinusAllMargins().right - axisMargin;
                nameBaselineTab[position] = calculator.getContentRectMinusAxesMargins().right + axisMargin
                        + labelTextAscentTab[position];
            } else {
                labelBaselineTab[position] = calculator.getContentRectMinusAxesMargins().right + axisMargin;
                nameBaselineTab[position] = labelBaselineTab[position] + axisMargin
                        + labelTextAscentTab[position] + labelDimensionForMarginsTab[position];
            }
            separationLineTab[position] = calculator.getContentRectMinusAllMargins().right;
        } else if (BOTTOM == position) {
            if (axis.isInside()) {
                labelBaselineTab[position] = calculator.getContentRectMinusAllMargins().bottom - axisMargin
                        - labelTextDescentTab[position];
                nameBaselineTab[position] = calculator.getContentRectMinusAxesMargins().bottom + axisMargin
                        + labelTextAscentTab[position];
            } else {
                labelBaselineTab[position] = calculator.getContentRectMinusAxesMargins().bottom + axisMargin
                        + labelTextAscentTab[position];
                nameBaselineTab[position] = labelBaselineTab[position] + axisMargin +
                        labelDimensionForMarginsTab[position];
            }
            separationLineTab[position] = calculator.getContentRectMinusAllMargins().bottom;
        } else if (TOP == position) {
            if (axis.isInside()) {
                labelBaselineTab[position] = calculator.getContentRectMinusAllMargins().top + axisMargin
                        + labelTextAscentTab[position];
                nameBaselineTab[position] = calculator.getContentRectMinusAxesMargins().top - axisMargin
                        - labelTextDescentTab[position];
            } else {
                labelBaselineTab[position] = calculator.getContentRectMinusAxesMargins().top - axisMargin
                        - labelTextDescentTab[position];
                nameBaselineTab[position] = labelBaselineTab[position] - axisMargin -
                        labelDimensionForMarginsTab[position];
            }
            separationLineTab[position] = calculator.getContentRectMinusAllMargins().top;
        } else {
        ...
        for (AxisValue axisValue : axis.getValues()) {
            ...
            if (value >= viewportMin && value <= viewportMax) {
                ...
                if (0 == valueIndex % module) {
                    if (isAxisVertical) {
                        rawValue = calculator.computeRawY(value);
                    } else {
                        rawValue = calculator.computeRawX(value);
                    }
    ...
    private void prepareAutoGeneratedAxis(Axis axis, int position) {
        ...
        for (int i = 0; i < autoValuesBufferTab[position].valuesNumber; ++i) {
            if (isAxisVertical) {
                rawValue = calculator.computeRawY(autoValuesBufferTab[position].values[i]);
            } else {
                rawValue = calculator.computeRawX(autoValuesBufferTab[position].values[i]);
            }
            if (checkRawValue(contentRect, rawValue, axis.isInside(), position, isAxisVertical)) {
                rawValuesTab[position][valueToDrawIndex] = rawValue;
    ...
    private void drawAxisLines(Canvas canvas, Axis axis, int position) {
        final Rect contentRectMargins = calculator.getContentRectMinusAxesMargins();
